...
export class MongoLib {
  ...
  private createIndexes() {
    ...
    this.tradeCollection.createIndex([{ symbol: 1 }, { timestamp: 1 }])

    this.markerCollection.createIndex([{ exchange: 1 }, { symbol: 1 }, { to: 1 }, { from: 1 }])

    this.orderCollection.createIndex([{ sessionId: 1 }, { symbol: 1 }, { timestamp: 1 }])

    this.walletCollection.createIndex([{ sessionId: 1 }, { exchange: 1 }, { symbol: 1 }, { strategy: 1 }])

    this.walletCollection.createIndex([
      { sessionId: 1 },
      { exchange: 1 },
      { symbol: 1 },
      { strategy: 1 },

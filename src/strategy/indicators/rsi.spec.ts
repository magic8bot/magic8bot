...
describe('RSI', () => {
  test('calculates the first averages correctly', () => {
  ...
  test('calculates rsi when avgLoss === 0', () => {
    jest.spyOn<any, any>(RSI, 'getAvgGainLoss').mockReturnValueOnce({ avgGain: 0, avgLoss: 0 })

    const { avgGain, avgLoss, rsi } = RSI.calculate(0, 0, candles.slice(1, 16), 14)
    expect(avgGain).toEqual(0)
    expect(avgLoss).toEqual(0)
    expect(rsi).toEqual(100)
  })

  test('calculates avgGain when currentGain === 0', () => {
    const { avgGain, avgLoss } = RSI.getAvgGainLoss(1, 1, [{ close: 1 }, { close: 2 }] as any, 1)
    expect(avgGain).toEqual(0)
    expect(avgLoss).toEqual(1)
  })
})
